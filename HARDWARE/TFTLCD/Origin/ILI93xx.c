...
{ 
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_14|GPIO_Pin_15;//PD0,1,4,5,7,8,9,10,11,14,15 AF OUT
...
{ 	 
	...
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这几个IC,则设置WR时序为最快
	{
		...
		FSMC_Bank1E->BWTR[6]|=3<<0;		//地址建立时间(ADDSET)为3个HCLK =18ns >	 15ns 	 
		FSMC_Bank1E->BWTR[6]|=2<<8; 	//数据保存时间(DATAST)为6ns*3个HCLK=18ns > 15ns
...
{          
 	...
 	}else
	{
		...
		for(i=sy;i<=ey;i++)
		{
			...
			for(j=0;j<xlen;j++)	LCD_RAM=color;
		}
	}	 
}  
//在指定区域内填充指定颜色块			 
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		for(j=0;j<width;j++) LCD_RAM=color[i*width+j];
	}		  
}  
//画线
//x1,y1:起点坐标
...
*Version:1.0 
	...
	for(t=0;t<pint;t++)
	{
	 	LCD_ShowChar(x+(size/2)*t,y,interger[pint-1-t]+'0',size,0);		
	}
	LCD_ShowChar(x+(size/2)*t,y,'.',size,0);
	for(t=0;t<precision;t++)
	{
		LCD_ShowChar(x+(size/2)*(pint+t+1),y,decimal[pdec-1-t]+'0',size,0);
	}
*/
...
void LCD_DrawCartesianCoordinate(u16 OriginX,u16 OriginY,char* LableX,char* LableY)
{
	...
	//Draw LableX
	LCD_ShowString(799-60,OriginY+XaxisHeight,240,24,24,(u8*)LableX,0);
	//Draw LableY
